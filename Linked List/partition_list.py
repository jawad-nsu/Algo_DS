...
class LinkedList:
    ...
    def partition_list(self, x):
        ...
        largerChainHead = Node(0)
        smallerChainHead = Node(0)
        
        largerChain = largerChainHead
        smallerChain = smallerChainHead
        
        current = self.head
        
        while current:
            if current.value < x:
                smallerChain.next = current
                smallerChain = current
            else:
                largerChain.next = current
                largerChain = current
                
            current = current.next
            
        largerChain.next = None
        smallerChain.next = largerChainHead.next
        self.head = smallerChainHead.next
